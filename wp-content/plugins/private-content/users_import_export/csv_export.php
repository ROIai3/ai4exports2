...
$indexes[] = array('index'=>'pc_csv_exp_export_type', 'label'=>esc_html__('Export type', 'pc_ml'), 'allowed'=>array('excel', 'csv'), 'required'=>true);
...
if(isset($_POST['pc_targeted_export_params'])) {
    ...
    $indexes[] = array('index'=>'pc_csv_exp_user_statuses', 'label'=>esc_html__('Users type', 'pc_ml'), 'allowed'=>array_keys($user_statuses), 'required'=>true);
    $indexes[] = array('index'=>'pc_csv_exp_user_cats', 'label'=>esc_html__('User categories', 'pc_ml'), 'allowed'=>array_keys($user_cats), 'required'=>true);
}

// PC-FILTER - allow CSV import form validation array management. Array elements must comply with Simple form validator class
$indexes = apply_filters('pc_csv_export_form_validation', $indexes);


if(isset($_POST['pc_csv_export'])) {
    ...
    // perform export
        ...
        $export_data = $uie->csv_export($fdata);
        
        if($export_data['status'] == 'error') {
            echo '<div class="notice notice-error"><p>'. wp_kses_post($export_data['message']) .'</p></div>';
        }
        else {
            $filename = 'pvtcontent_on_'. strtolower(sanitize_title($_SERVER['HTTP_HOST'])) .' -'. wp_date('Y-m-d-H:i:s') .'.';
            $filename .= ($fdata['pc_csv_exp_export_type'] == 'csv') ? 'csv' : 'xlsx';
            
            $mime = ($fdata['pc_csv_exp_export_type'] == 'csv') ? 
            ...
            (function() { 
                ...
                let byteString = atob(`'. pc_static::wp_kses_ext(str_replace('`', '\`', $export_data['filedata'])) .'`),
                ...
                const blob = new Blob([ab], {type: `<?php echo esc_js($mime) ?>`}),
...
<div class="pc_imp_exp_box">
    ...
    <form method="post" class="form-wrap" action="<?php echo esc_attr(pc_static::curr_url()) ?>">
            ...
            <tbody>
                ...
                <?php if(!isset($_GET['pc_targeted_export']) || !isset($_GET['pc_texp_label']) || !isset($_GET['pc_texp_conds'])) : ?>
                    <tr>
                        ...
                        <td>
                            <select name="pc_csv_exp_user_statuses[]" class="pc_lc_select" data-placeholder="<?php esc_attr_e('Select an option', 'pc_ml') ?> .." multiple="multiple" autocomplete="off">
                                <?php
                                foreach($user_statuses as $key => $label) {
                                    $sel = (is_array($fdata['pc_csv_exp_user_statuses']) && in_array($key, $fdata['pc_csv_exp_user_statuses'])) ? 'selected="selected"' : '';
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="pc_csv_exp_user_cats[]" class="pc_lc_select" data-placeholder="<?php esc_attr_e('Select categories', 'pc_ml') ?> .." multiple="multiple" autocomplete="off">
                                <?php
                                foreach($user_cats as $key => $label) {
                                    $sel = (is_array($fdata['pc_csv_exp_user_cats']) && in_array($key, $fdata['pc_csv_exp_user_cats'])) ? 'selected="selected"' : '';
                                    echo '<option value="'. esc_attr($key) .'" '. esc_html($sel) .'>'. esc_html($label) .'</option>'; 
                                }
                                ?>
                            </select>
                ...
                <?php else : ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="hidden" name="pc_targeted_export_params" value="<?php echo pc_static::sanitize_val($_GET['pc_targeted_export']) ?>" />
                        </td>
                    </tr>
                <?php endif; ?>
                
                <tr>
                    <td><?php esc_html_e("Export as", 'pc_ml') ?></td>
                    <td>
                        <select name="pc_csv_exp_export_type" class="pc_lc_select" data-placeholder="<?php esc_attr_e('Select an option', 'pc_ml') ?> .." autocomplete="off">
                            <option value="excel"><?php esc_html_e('Excel', 'pc_ml') ?> (.xls)</option>
                            <option value="csv" <?php selected('csv', $fdata['pc_csv_exp_export_type']) ?>>CSV</option>
                        </select>
                    </td>
                </tr>
                ...
                do_action('pc_csv_export_form', $fdata);
                ?>
        ...
        <input type="submit" name="pc_csv_export" value="<?php esc_attr_e('Export', 'pc_ml') ?>" class="button-primary" />  
    </form>
</div>
...
(function($) { 
    ...
    $(document).on(`change`, `select[name="pc_csv_exp_user_cats[]"], select[name="pc_csv_exp_user_statuses[]"]`, function() {
