(function($) { 
	...
	var atts = {};
	$.each(pc_registr_defaults, function(i, v) {
	...
	window.pc_registr_guten_on_display = function(blockId) {
		setTimeout(function() { // wait a bit for possible guten mess
			
			if(!$('#block-'+ blockId +' .pc_registration_form').length || $('#block-'+ blockId +' .components-placeholder').length) {
				setTimeout(function() {
					pc_registr_guten_on_display(blockId);
				}, 350);
				return false;
			}
			...
			pc_checkboxes_setup();
			pc_lc_select_setup();
			$(window).trigger('resize'); // trick showing the form
			
			// fix strange CSS issues
			if(!$('#pc_registr_css_fix').length) {
				$('head').append('<style type="text/css" id="pc_registr_css_fix">.pc_check_wrap .pc_check_label {float: none;}</style>');
			}
		}, 400);
	};
	...
	// register block
	var args = {
		block_id			: 'lcweb/pc-registration-form',
		category			: 'lc-pvtcontent',
		title				: wp.i18n.__('Registration Form', 'pc_ml'),
		icon				: icon,
		panels				: pc_panels,
		structure			: pc_registr_defaults,
		attributes			: atts,
		on_display_callback : 'pc_registr_guten_on_display',
	};
