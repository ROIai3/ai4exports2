(function($) { 
	...
	var atts = {};
	$.each(pc_login_defaults, function(i, v) {
	...
	window.pc_login_guten_on_display = function(blockId) {
		setTimeout(function() { // wait a bit for possible guten mess
			
			if(!$('#block-'+ blockId +' .pc_login_form').length || $('#block-'+ blockId +' .components-placeholder').length) {
				setTimeout(function() {
					pc_login_guten_on_display(blockId);
				}, 350);
				return false;
			}
	...
	// register block
	var args = {
		block_id			: 'lcweb/pc-login-form',
		category			: 'lc-pvtcontent',
		title				: wp.i18n.__('Login Form', 'pc_ml'),
		icon				: icon,
		panels				: pc_panels,
		structure			: pc_login_defaults,
		attributes			: atts,
		on_display_callback : 'pc_login_guten_on_display',
	};
