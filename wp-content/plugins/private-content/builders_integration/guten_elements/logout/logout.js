(function($) { 
	...
	var atts = {};
	$.each(pc_logout_defaults, function(i, v) {
	...
	window.pc_logout_guten_on_display = function(blockId) {
		setTimeout(function() { // wait a bit for possible guten mess
			
			if(!$('#block-'+ blockId +' .pc_logout_btn').length || $('#block-'+ blockId +' .components-placeholder').length) {
				setTimeout(function() {
					pc_logout_guten_on_display(blockId);
				}, 350);
				return false;
			}
		}, 400);
	...
	// register block
	var args = {
		block_id			: 'lcweb/pc-logout-box',
		category			: 'lc-pvtcontent',
		title				: wp.i18n.__('Logout Box', 'pc_ml'),
		icon				: icon,
		panels				: pc_panels,
		structure			: pc_logout_defaults,
		attributes			: atts,
		on_display_callback : 'pc_logut_guten_on_display',
	};
