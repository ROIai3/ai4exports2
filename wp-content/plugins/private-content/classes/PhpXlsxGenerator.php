 ...
 ** #### AUTHOR LINES EDITED BY LCWEB ##########
 */
if(!defined('ABSPATH')) {exit;}


if(!class_exists('PhpXlsxGenerator')) {
    class PhpXlsxGenerator
    {
        public $curSheet;
        ...
        {
            $author = 'PrivateContent - LCweb <support@lcweb.it>';
    ...
    {TYPES}
        ...
        protected function _writeEntry($fh, &$cdrec, $cfilename, $data)
        {
            ...
            $lastmod_timeS = str_pad(decbin(gmdate('s') >= 32 ? gmdate('s') - 32 : gmdate('s')), 5, '0', STR_PAD_LEFT);
            $lastmod_timeM = str_pad(decbin(gmdate('i')), 6, '0', STR_PAD_LEFT);
            $lastmod_timeH = str_pad(decbin(gmdate('H')), 5, '0', STR_PAD_LEFT);
            $lastmod_dateD = str_pad(decbin(gmdate('d')), 5, '0', STR_PAD_LEFT);
            $lastmod_dateM = str_pad(decbin(gmdate('m')), 4, '0', STR_PAD_LEFT);
            $lastmod_dateY = str_pad(decbin(gmdate('Y') - 1980), 7, '0', STR_PAD_LEFT);
