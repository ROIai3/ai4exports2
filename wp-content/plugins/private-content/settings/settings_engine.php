...
if(!class_exists('lcwp_settings_engine')) {
class lcwp_settings_engine {
    
    public static $prod_acronym; // (string)
	public static $css_prefix; // (string) prefix added to classes for customized styling
	public static $ml_key; // (string) multilanguage key
	...
	public function __construct($submit_btn_name, $tabs, $structure) {
        ...
        // set baseurl
        $uri = explode('wp-admin/', sanitize_text_field($_SERVER['REQUEST_URI']))[1];
        $clean_url = explode('?', admin_url($uri))[0]; 
        
        $this->baseurl = add_query_arg(array('page' => sanitize_text_field($_GET['page'])), $clean_url);
        if(isset($_GET['post_type'])) {
            $this->baseurl = add_query_arg(array('post_type' => sanitize_text_field($_GET['post_type'])), $this->baseurl);        
        }
	...
	public function get_code() {
		$form_action = str_replace(array('%7E', '&lcwp_sf_success'), array('~', ''), sanitize_text_field($_SERVER['REQUEST_URI']));
        
		echo '
        <form name="'. esc_attr(self::$css_prefix) .'settings_form" method="post" class="lcwp_settings_form '. esc_attr(self::$css_prefix) .'settings_form form-wrap" action="'. esc_attr($form_action) .'" novalidate>';
		
			// tabs
			echo self::wp_kses_ext($this->tabs_code());
			
			// sections and fields
			foreach($this->tabs as $tab_id => $tab_name) {
				if(!isset($this->structure[ $tab_id ])) {
                    continue;
                }
				
				echo '<div id="'. esc_attr($tab_id) .'" class="lcwp_settings_block '. esc_attr(self::$css_prefix) .'settings_block">';
				...
				foreach($this->structure[ $tab_id ] as $sect_id => $section) {
                    ...
                    if($section['sect_name']) {
					   echo '<h3 class="lcwp_settings_sect_title '. esc_attr(self::$css_prefix) .'settings_sect_title" data-sect-id="'. esc_attr($sect_id) .'">'. self::wp_kses_ext($section['sect_name']) .'</h3>';
                    }
					...
					if($use_table) {
						echo '<table class="widefat lcwp_settings_table '. esc_attr(self::$css_prefix) .'settings_block" data-sect-id="'. esc_attr($sect_id) .'"><tbody>';
					}
			...
			// nonce & submit button
			echo '
			<input type="hidden" name="'. esc_attr(self::$css_prefix) .'settings_nonce" value="'. esc_attr(wp_create_nonce('lcwp')) .'" /> 
			<input type="submit" name="'. esc_attr($this->submit_btn_name) .'" value="'. esc_attr__('Update Options', self::$ml_key) .'" class="button-primary lcwp_settings_submit" />
	...
	public function opt_to_code($field_id, $field) {	
		...
		if($f['type'] == 'spacer') {
			...
			echo '<tr class="lcwp_sf_tr '. esc_attr(self::$css_prefix . $field_id .' '. $tr_hidden) .'"><td class="lcwp_sf_spacer" data-field-id="'. esc_attr($field_id) .'" colspan="3"></td></tr>';
		...
		if($f['type'] == 'message') {
			$tr_hidden = (isset($f['hide']) && $f['hide']) ? 'lcwp_sf_displaynone' : '';
			echo '<tr class="lcwp_sf_tr '. esc_attr(self::$css_prefix . $field_id .' '. $tr_hidden) .'"><td class="lcwp_sf_message" data-field-id="'. esc_attr($field_id) .'" colspan="3">'. self::wp_kses_ext($f['content']) .'</td></tr>';
		...
		if($f['type'] == 'label_message') {
			$tr_hidden = (isset($f['hide']) && $f['hide']) ? 'lcwp_sf_displaynone' : '';
			echo '
			<tr class="lcwp_sf_tr '. esc_attr(self::$css_prefix . $field_id .' '. $tr_hidden) .'">
				<td class="lcwp_sf_label"><label>'. self::wp_kses_ext($f['label']) .'</label></td>
				<td class="lcwp_sf_message" data-field-id="'. esc_attr($field_id) .'" colspan="2">'. self::wp_kses_ext($f['content']) .'</td>
			</tr>';
        ...
        // CONTEXTUAL HELP
		if($f['type'] == 'context_help') {
            $tr_hidden = (isset($f['hide']) && $f['hide']) ? 'lcwp_sf_displaynone' : '';
            
            $arrow_pos = 'top';
            if(isset($f['arrow_pos']) && $f['arrow_pos'] == 'bottom') {
                $arrow_pos = 'botttom';   
            }
            elseif(isset($f['arrow_pos']) && $f['arrow_pos'] == 'none') {
                $arrow_pos = 'none';   
            }
            
			echo '
			<tr class="lcwp_sf_fullw_helper_tr '. esc_attr(self::$css_prefix . $field_id .' '. $tr_hidden) .'" data-arrow-pos="'. esc_attr($arrow_pos) .'" data-linked-f="'. esc_attr(self::$css_prefix .  $f['linked_field']) .'">
				<td colspan="3">
                    <div class="lcwp_sf_fullw_helper" data-helper-name="'. esc_attr($field_id) .'"><div>'. self::wp_kses_ext($f['content']) .'</div></div>
                </td>
			</tr>';
		...
		$tr_hidden = (isset($f['hide']) && $f['hide']) ? 'lcwp_sf_displaynone' : '';
		echo '<tr class="lcwp_sf_tr '. esc_attr(self::$css_prefix . $field_id .' '. $tr_hidden) .'">';
		...
		if($f['type'] == 'code_editor') {
			...
			// label block
			echo '<td class="lcwp_sf_label"><label>'. self::wp_kses_ext($f['label']) .'</label></td>';
		...
		switch($f['type']) {
			...
			case 'text' :
				...
				echo '<input type="text" name="'. esc_attr($field_id) .'" value="'. esc_attr((string)$val) .'" '. esc_html($ml) .' placeholder="'. esc_attr($ph) .'" autocomplete="off" />';
				break;
			...
			case 'psw' :
				...
				if(!empty($val)) {
                    ...
                    <div class="lcwp_sf_edit_psw">
						...
						<a role="button" data-fid="'. esc_attr($field_id) .'" title="'. esc_html__('change password', self::$ml_key) .'"><span class="dashicons dashicons-edit"></span></a>
					...
					if(!isset($GLOBALS['lcwp_sf_edit_psw_js'])) {
                        ...
                        (function($){
                        	...
                        	$(document).on(`click`, `.lcwp_sf_edit_psw a`, function() {
								 $(this).parents(`.lcwp_sf_edit_psw`).replaceWith(`<input type="password" name="`+ $(this).data(`fid`) +`" value="" autocomplete="off" />`);
							});  
                        })(jQuery);';
					...
					echo '<input type="password" name="'. esc_attr($field_id) .'" value="" autocomplete="off" />';
				}
			...
			case 'select' :
				...
				echo '
				<select data-placeholder="'. esc_attr__('Select an option', self::$ml_key) .' .." name="'. esc_attr($field_id . $multiple_name) .'" class="lcwp_sf_select" autocomplete="off" '. esc_html($multiple_attr) .'>';
				...
				foreach((array)$f['val'] as $key => $name) {
					...
					echo '<option value="'. esc_attr($key) .'" '. esc_html($sel) .'>'. esc_html($name) .'</option>';	
				}
			...
			case 'checkbox' :
				...
				echo '
				<input type="checkbox" name="'. esc_attr($field_id) .'" value="1" class="lcwp_sf_check" '. esc_html($sel) .' autocomplete="off" />';
				break;
			...
			case 'textarea' :
				$ph = (isset($f['placeh'])) ? $f['placeh'] : ''; 
				echo '
				<textarea name="'. esc_attr($field_id) .'" placeholder="'. esc_attr($ph) .'" class="lcwp_sf_textarea" onkeyup="lcwp_sf_textAreaAdjust(this)" autocomplete="off">'. esc_textarea($val) .'</textarea>';
				break;
			...
			case 'code_editor' :
				echo '
				<textarea id="'. esc_attr($field_id) .'" name="'. esc_attr($field_id) .'" autocomplete="off" class="lcwp_sf_code_editor" data-language="'. esc_attr($f['language']) .'">'. esc_textarea($val) .'</textarea>';
				break;	
			...
			case 'slider' :
				...
				echo '
            	<input type="number" value="'. floatval($val) .'" name="'. esc_attr($field_id) .'" min="'. esc_attr($f['min_val']) .'" max="'. esc_attr($f['max_val']) .'" step="'. esc_attr($f['step']) .'" class="lcwp_sf_slider_input" autocomplete="off" data-unit="'. esc_attr($f['value']) .'" data-respect-limits="'. absint($respect_limits) .'" />';
				break;
			...
			case 'color' :
                ...
                echo '
				<input type="text" name="'. esc_attr($field_id) .'" value="'. esc_attr($val) .'" class="lcwp_sf_colpick" data-modes="'. esc_attr(implode(' ', $modes)) .'" data-def-color="'. esc_attr($def_color) .'" autocomplete="off" />';
				break;
			...
			case '2_numbers' :
				...
				for($a=0; $a<2; $a++) {
					echo '<input type="number" name="'. esc_attr($field_id) .'[]" value="'. absint($val[$a]) .'" '. esc_html($min) .' '. esc_html($max) .' class="lcwp_sf_2num_input"  autocomplete="off" />' ;	
				}
				
				if(isset($f['value'])) {
					echo ' <span>'. self::wp_kses_ext($f['value']) .'</span>';
				}
				break;
			...
			case '4_numbers' :
				...
				$min    = (isset($f['min_val'])) ? 'min="'. absint($f['min_val']) .'"' : '';
				$max    = (isset($f['max_val'])) ? 'max="'. absint($f['max_val']) .'"' : '';
				
				for($a=0; $a<4; $a++) {
					echo '<input type="text" name="'. esc_attr($field_id) .'[]" value="'. absint($val[$a]) .'" '. esc_html($min) .' '. esc_html($max) .' class="lcwp_sf_4num_input" autocomplete="off" />' ;	
				}
		...
		if($is_fullwidth) {
			echo '
				'. self::wp_kses_ext($note) .'
			</td>';	
		}
		else {
			echo '
			</td>
			<td>'. self::wp_kses_ext($note) .'</td>';	
		}
        ...
        (function($) { 
            ...
            foreach($this->js_vis_cond as $field_id => $data) {
                ...
                $(document).on(
                    ...
                    `[name="'. esc_js($data['linked_field']) .'"], [name="'. esc_js($data['linked_field']) .'[]"]`, 
                    '. wp_json_encode($data) .',
                    function(e) {
                        let $linked = $(this),
                            $field_wrap = $(`[name=`+ e.data.field_id +`], [name="`+ e.data.field_id +`[]"], [data-field-id="`+ e.data.field_id +`"], [data-helper-name="`+ e.data.field_id +`"]`).parents(`tr`).first(),
                        ...
                        const field_helper_selector = `.lcwp_sf_fullw_helper_tr[data-linked-f="`+ $field_wrap.attr(`class`).replace(`lcwp_sf_displaynone`, ``).trim() +`"]`;
                        if(show) {
                            $field_wrap.removeClass(`lcwp_sf_displaynone`);
                            $(field_helper_selector).removeClass(`lcwp_sf_helper_displaynone`);
                        }
                        else {
                            $field_wrap.addClass(`lcwp_sf_displaynone`);
                            $(field_helper_selector).addClass(`lcwp_sf_helper_displaynone`);
                        }       		
                    }
                );

                // trigger on page opening
                $(`[name="'. esc_js($data['linked_field']) .'"]`).trigger(`change`).trigger(`lcs-statuschange`);';
	...
	private function get_fields_validation() {
		...
		foreach($this->fields as $fid => $fval) {
			if(in_array($fval['type'], array('spacer', 'message', 'label_message', 'context_help')) || isset($fval['no_save'])) {
                continue;
            }
	...
	public function validate() {
		...
		$validator = new simple_fv(self::$ml_key);
		...
		if(!isset($_POST[$noncename]) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST[$noncename])), 'lcwp')) {
		...
		foreach($fdata as $key => $val) {
			if(!is_array($val)) {
				...
				foreach($val as $arr_key => $arr_val) {
					$fdata[$key][$arr_key] = (is_array($arr_val)) ? $arr_val : stripslashes($arr_val);
				}
			}
		}
	...
	public function save_data() {
		foreach($this->form_data as $key => $val) {
            ...
            switch($f_type) {
                ...
                case 'text' :
                    $val = ($val != wp_strip_all_tags($val)) ? self::wp_kses_ext($val) : sanitize_text_field($val);
                    break;
                    
                case 'textarea' :
                    $val = ($val != wp_strip_all_tags($val)) ? self::wp_kses_ext($val) : sanitize_textarea_field($val);
                    break;
                    
                case 'code_editor' :
                case 'wp_editor' :
                    $val = self::wp_kses_ext($val);
                    $val = str_replace(array('&gt;', '&lt;'), array('>', '<'), $val);
                    break;
                    
                default :
                    $val = (is_array($val)) ? $this->recursive_wp_kses_post($val) : self::wp_kses_ext($val);
                    break;
            }
    ...
    private function recursive_wp_kses_post($data) {
        foreach($data as $k => $v) {
            $data[$k] = (is_array($v)) ? $this->recursive_wp_kses_post($v) : self::wp_kses_ext($v);
        }
        return $data;
    }
    ...
    public function successful_save_redirect() {
        ...
        if(isset($_GET['lcwpTopScroll']) && (int)$_GET['lcwpTopScroll']) {
            $redir_url .= '&lcwpTopScroll='. absint($_GET['lcwpTopScroll']);
        }
    ...
    public function handle_import_data() {
        $import_str = sanitize_text_field($_GET['lcwp_sf_import']);
        $nonce_attr = self::$css_prefix .'settings_nonce';
        
        if(substr($import_str, 0, 4) == 'gzc_') {
            if(!function_exists('gzuncompress')) {
                echo self::wp_kses_ext($this->get_error_message_html(array(
                    esc_html__('PHP compressed string passed but gzuncompress module not enabled. Please enable it in your PHP configuration', self::$ml_key)
                )));
                return false;
            }
                
            $import_str = substr($import_str, 3);
            $data = json_decode(gzuncompress(base64_decode($import_str)), true); 
        }
        else {
            $data = json_decode(base64_decode($import_str), true);
        }
        ...
        $_POST[$nonce_attr] = (isset($_GET[$nonce_attr])) ? sanitize_text_field($_GET[$nonce_attr]) : false;     
    ...
    public static function get_final_export_string($json_data) {
        if(function_exists('gzcompress') && !isset($_GET['lcwp_no_compress_export'])) {
            return 'gzc_'. base64_encode(gzcompress($json_data, 9));
        } else {
    ...
    public function import_export_btns($cust_db_opt_keys_to_export = array()) {
        ...
        $json_data = $this->get_export_json($cust_db_opt_keys_to_export);
        $export_string = self::get_final_export_string($json_data);
        ...
        (function($) { 
            ...
            const import_baseurl = `'. esc_attr($redir_url) .'`;
        
        
            // export data
            $(document).on("click", ".lcwp_sf_export_btn", function() {
                const f = document.createElement("input");
                f.setAttribute("value", `'. esc_js($export_string) .'`);
            ...
            // import data
            $(document).on("click", ".lcwp_sf_import_btn", function() {
                ...
                window.location.href = import_baseurl +"="+ encodeURIComponent(json_data.trim()) +"&lcwp_sf_is_importing&'. esc_js(self::$css_prefix) .'settings_nonce='. esc_js(wp_create_nonce('lcwp')) .'";
            });
